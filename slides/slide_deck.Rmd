...
- That having students turn in assignments using R Markdown provides an efficient way to check their work and their analyses easily.
...
- <small><small><small>Note that you may need to refresh the page for the data to appear.  Not available in PDF.</small></small></small>

```{r rows.print=7}
library(fivethirtyeight); library(readr); library(rmarkdown)
...
- Use the `readr`, `readxl`, and `haven` packages to read in data from CSVs, Excel XLS files, STATA/SPSS files

- Use `ggplot2` and `dplyr` to make graphics and produce data wrangling on data sets of interest to you
